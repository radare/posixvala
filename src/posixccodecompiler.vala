 ...
 */
public class Vala.PosixCCodeCompiler : Vala.CCodeCompiler {
	public PosixCCodeCompiler () {
	}
	 ...
	 */
	public new void compile (CodeContext context, string? cc_command, string[] cc_options) {
		...
		foreach (string pkg in context.get_packages ()) {
			if (package_exists (pkg)) {
				/* HACK: glib banned */
				if (pkg != "glib-2.0" && pkg != "gobject-2.0") {
					use_pkgconfig = true;
					pc += " " + pkg;
				}
			}
